...
export const Loading: React.FC<ILoadingProps> = ({
  ...
  return (
    <Element
      className={cn(
        'relative size-10 animate-spin rounded-full border-[3px] border-transparent border-t-[#0da7e9]',
        className,
      )}
      style={style ?? {}}
      {...rest}
    >
      <div
        className={cn(
          'absolute inset-0 animate-spin rounded-full border-2 border-transparent border-r-[#6e6e6e]/55 border-t-[#6e6e6e]/55',
          insideClassName,
        )}
      />
    </Element>
